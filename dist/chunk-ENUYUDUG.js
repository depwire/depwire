...
import { extname as extname12, basename as basename11 } from "path";
...
import { join as join22, resolve as resolve15, basename as basename12 } from "path";
...
function detectSubprocessEdges(files, projectRoot) {
  ...
  for (const f of files) {
    const base = basename12(f.filePath);
  ...
  for (const file of files) {
    ...
    for (const call of calls) {
      ...
      if (knownFiles.has(call.calledFile)) {
        ...
        const base = basename12(call.calledFile);
...
function calculateOrphansScore(graph, projectRoot) {
  const files = /* @__PURE__ */ new Set();
...
function isBarrelFile(filePath) {
  const basename16 = path3.basename(filePath);
  return basename16 === "index.ts" || basename16 === "index.js";
}
...
import { basename as basename13, extname as extname13 } from "path";
...
function generateFileOrganization(graph) {
  ...
  graph.forEachNode((node, attrs) => {
    if (!files.has(attrs.filePath)) {
      files.add(attrs.filePath);
      const fileName = basename13(attrs.filePath);
...
function generateNamingPatterns(graph) {
  ...
  graph.forEachNode((node, attrs) => {
    if (!files.has(attrs.filePath)) {
      files.add(attrs.filePath);
      const fileName = basename13(attrs.filePath, extname13(attrs.filePath));
...
// src/docs/files.ts
import { dirname as dirname22, basename as basename14 } from "path";
...
function generateDirectoryBreakdown(graph) {
  ...
  for (const file of fileStats) {
    ...
    if (file.totalConnections > dirStats.maxConnections) {
      ...
      dirStats.mostConnectedFile = basename14(file.filePath);
    }
  }
  if (dirMap.size === 0) {
...
// src/docs/tests.ts
import { basename as basename15, dirname as dirname23 } from "path";
...
function isTestFile3(filePath) {
  const fileName = basename15(filePath).toLowerCase();
...
function matchTestToSource(testFile) {
  const testFileName = basename15(testFile);
...
function generateTestCoverageMap(graph) {
  ...
  const rows = mappings.slice(0, 30).map((m) => [
    `\`${m.sourceFile}\``,
    m.hasTest ? "\u2705" : "\u274C",
    m.testFile ? `\`${basename15(m.testFile)}\`` : "-",
